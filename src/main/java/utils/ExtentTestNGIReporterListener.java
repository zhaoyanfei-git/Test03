...
public class ExtentTestNGIReporterListener implements IReporter {
    ...
    public void generateReport(List<XmlSuite> xmlSuites, List<ISuite> suites, String outputDirectory) {
        ...
        for (ISuite suite : suites) {
            ...
            // 如果 config 里面没有任何用例，直接跳过，不在报告里生成
            if (result.size() == 0) {
                continue;
            }
            // 统计 config 下的成功、失败、跳过的总用例数
            int suiteFailSize = 0;
            int suitePassSize = 0;
            int suiteSkipSize = 0;
            ExtentTest suiteTest = null;
            // 存在多个 config 的情况下，在报告中将同一个一个 config 的测试结果归为一类，创建一级节点。
            if (createSuiteNode) {
            ...
            for (ISuiteResult r : result.values()) {
                ...
                if (createSuiteResultNode) {
                    // 没有创建 config 的情况下，将在 SuiteResult 的创建为一级节点，否则创建为 config 的一个子节点。
                    if (null == suiteTest) {
...
//        extentXReporter.config().setProjectName("test1");
//        extentXReporter.config().setReportName("Build-1224");
